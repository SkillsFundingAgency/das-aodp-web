...
{
    public class CheckboxValidator : IAnswerValidator
    {
        ...
        public void Validate(GetApplicationPageByIdQueryResponse.Question question, ApplicationPageViewModel.Answer? answer)
        {
            ...
            if (min is not null && (min > answer!.MultipleChoiceValues?.Count))
                throw new QuestionValidationFailedException(question.Id, question.Title, $"Please select at least {min} options.");

            if (max is not null && (max < answer!.MultipleChoiceValues?.Count))
