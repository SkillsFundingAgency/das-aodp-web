...
{
    public class RolloverEligibilityDatesViewModelValidator : AbstractValidator<RolloverEligibilityDatesViewModel>
    {
        public RolloverEligibilityDatesViewModelValidator()
        {
            RuleFor(x => x.FundingEndDate)
                .NotNull()
                .WithMessage("Funding end date is required");

            RuleFor(x => x.OperationalEndDate)
                .NotNull()
                .WithMessage("Operational end date is required");

            RuleFor(x => x).Custom((m, ctx) =>
            {
                if (m.FundingEndDate != null)
                    ValidateDateGroup(m.FundingEndDate, "FundingEndDate", "Funding end date", ctx);

                if (m.OperationalEndDate != null)
                    ValidateDateGroup(m.OperationalEndDate, "OperationalEndDate", "Operational end date", ctx);
            });
        }

        private static void ValidateDateGroup(RolloverEligibilityDate dateModel, string fieldPrefix, string fieldName, ValidationContext<RolloverEligibilityDatesViewModel> ctx)
        {
            ...
            {
                ctx.AddFailure(fieldPrefix, $"{fieldName} must be today or in the future");
                ctx.AddFailure($"{fieldPrefix}.Day", "Enter a valid day");
                ctx.AddFailure($"{fieldPrefix}.Month", "Enter a valid month");
                ctx.AddFailure($"{fieldPrefix}.Year", "Enter a valid year");
            }
