...
{
    public class WhenHandlingDeleteFileMetadataCommand
    {
        private const string ExpectedErrorMessage = "api failed";

        private readonly Fixture _fixture = new();
        private readonly Mock<IApiClient> _apiClient = new();
        private readonly DeleteFileMetadataCommandHandler _handler;

        public WhenHandlingDeleteFileMetadataCommand()
        {
        ...
        {
            // Arrange
            var request = _fixture.Create<DeleteFileMetataCommand>();
            ...
            Assert.Multiple(() =>
            {
                _apiClient.Verify(a =>
                    a.PostWithResponseCode<EmptyResponse>(
                        It.Is<DeleteFileMetadataApiRequest>(r => r.Data == request)),
                    Times.Once);
